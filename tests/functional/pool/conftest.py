...
@pytest.fixture(scope="module")
def callee_for_reentrancy_with_open_and_receive(
    project, accounts, token0, token1, sender
):
    callee_reentrancy = (
        project.TestMarginalV1PoolReentrancyWithOpenAndReceiveCallee.deploy(
            sender=accounts[0]
        )
    )
    token0.approve(callee_reentrancy.address, 2**256 - 1, sender=sender)
