...
contract MarginalV1Factory {
    ...
    event PoolCreated(address token0, address token1, uint256 maintenance, address pool);
    ...
    function createPool(
        ...
        pool = address(new MarginalV1Pool{salt: keccak256(abi.encode(token0, token1, maintenance))}());
